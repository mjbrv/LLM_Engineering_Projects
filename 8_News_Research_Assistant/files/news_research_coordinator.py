...
"""
        ...
        # Fact Check Summary
        md_content += "\n## Fact Check Summary\n"
        fact_check = detailed.get('fact_check_summary', {})
        ...
        md_content += "\n### Disputed Claims\n"
        for claim in fact_check.get('disputed_claims', []):
            md_content += f"- {claim}\n"
        
        md_content += "\n### Unverified Claims\n"
        for claim in fact_check.get('unverified_claims', []):
            md_content += f"- {claim}\n"
    ...
    async def research_topic(self, topic: str, time_range: Dict[str, str] = None) -> Dict:
        ...
        try:
            ...
            fact_check_pbar = tqdm(total=1, desc="✔️ Fact checking articles", position=2, leave=True)
            report_pbar = tqdm(total=1, desc="📝 Generating final report", position=3, leave=True)
            ...
            # Step 3: Fact check
            fact_check_results = await self.fact_check_agent.process(articles)
            fact_check_pbar.update(1)
            fact_check_pbar.close()
